...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest }) => {
  const [activeTab, setActiveTab] = useState<StudentTab | 'LEADERBOARD' | 'STORE' | 'VIDEO' | 'PDF' | 'MCQ'>('ROUTINE');
  ...
  const claimYesterdayReward = () => {
      ...
      const updatedUser: User = { 
          ...user, 
          subscriptionTier: showRewardModal.tier as any, 
  ...
  return (
    <div>
        ...
        <div className={`grid ${isGameEnabled ? 'grid-cols-8' : 'grid-cols-7'} gap-1 bg-white p-2 rounded-xl border border-slate-200 shadow-sm mb-8 sticky top-20 z-20 overflow-x-auto`}>
            <button onClick={() => {setActiveTab('ROUTINE'); setContentViewStep('SUBJECTS');}} className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all min-w-[60px] ${activeTab === 'ROUTINE' ? 'bg-slate-100 text-[var(--primary)]' : 'text-slate-400 hover:bg-slate-50'}`}><Calendar size={18} className="mb-1" /><span className="text-[9px] font-bold uppercase">Routine</span></button>
